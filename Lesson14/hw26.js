...
//HW 26. Реалізувати подобу інтернет-магазину
//Дано 3 блоки
//В лівій частині сторінки - перелік категорій. При натисканні на категорію в 
//середній блок виводиться список товарів цієї категорії. Клік на товар - 
//інформацію про товар у правому блоці. В інформації товару - кнопка “купити”. 
...
const categoryClick = document.querySelector("#categories");
const productClick = document.querySelector("#products");
...
const handleCategoryClick = (ev) => {
  ...
  const clear = document.querySelector ("#products");
  while(clear.firstChild){
    clear.removeChild(clear.firstChild);
  }                 
                        
  const currentCategory = ev.target.dataset.categoryId;
  const productsCurrentCategory = products.filter(item => item.category_id == currentCategory);
  console.log(productsCurrentCategory)
  const productsUl = document.querySelector ("#products");

  for (let i = 0; i < productsCurrentCategory.length; i++){
  const li = document.createElement("li");
  li.innerText = productsCurrentCategory[i].name
...
const handleProductClick = (ev) => {
  ...
  const currentProduct = ev.target.innerText;
  const arrDescription = products.filter(item => item.name == currentProduct);
  description.innerText = arrDescription[0].description
}
...
categoryClick.addEventListener ("click",  handleCategoryClick);
productClick.addEventListener ("click",  handleProductClick);
buttonBuy.addEventListener ("click",  handleButtonBuyClick);